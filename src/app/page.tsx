...
export default function Home() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        if (data.error === 'Database not configured') {
          showToast('⚙️ Please configure MONGODB_URI in your .env.local file', 'error')
        } else {
      ...
      showToast('Connection error. Please check your database configuration.', 'error')
      setFoods([])
    }
    setLoading(false)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(editingFood ? 'Food updated successfully!' : 'Food added successfully!', 'success')
        ...
        <div className="bg-white/80 backdrop-blur-sm p-4 sm:p-6 lg:p-8 rounded-2xl shadow-xl border border-white/20 mb-6 lg:mb-8">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
            ...
            <div className="sm:col-span-2 flex flex-col sm:flex-row gap-3 lg:gap-4">
              <button
                type="submit"
                className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-6 lg:px-8 py-3 lg:py-4 rounded-xl hover:from-purple-700 hover:to-blue-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1 text-sm sm:text-base"
              >
        ...
        <div className="bg-white/80 backdrop-blur-sm p-4 sm:p-6 lg:p-8 rounded-2xl shadow-xl border border-white/20">
          ...
          {loading ? (
              ...
              {filteredFoods.map((food) => (
                <div key={food._id} className="bg-gradient-to-br from-white to-gray-50 p-4 lg:p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                  <div className="mb-4">
                    ...
                    <p className="text-gray-600 leading-relaxed text-sm lg:text-base line-clamp-3">{food.description}</p>
                  </div>
                  ...
                  <div className="flex gap-2 lg:gap-3 mt-4 lg:mt-6">
                    ...
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-red-500 to-pink-600 text-white px-3 lg:px-4 py-2 lg:py-3 rounded-xl text-xs lg:text-sm font-semibold hover:from-red-600 hover:to-pink-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1"
                    >
                      🗑️ Delete
                    </button>
                  </div>
            ...
            <div className="space-y-4">
              {filteredFoods.map((food) => (
                <div key={food._id} className="bg-gradient-to-r from-white to-gray-50 p-4 lg:p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2 lg:gap-3 sm:flex-col sm:w-32">
                      ...
                      <button
                        onClick={() => deleteFood(food._id!)}
                        className="flex-1 sm:flex-none bg-gradient-to-r from-red-500 to-pink-600 text-white px-3 lg:px-4 py-2 lg:py-3 rounded-xl text-xs lg:text-sm font-semibold hover:from-red-600 hover:to-pink-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1"
                      >
                        🗑️ Delete
                      </button>
                    </div>
